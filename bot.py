  ...
  if event.is_private:
    async for usr in client.iter_participants(event.chat_id):
     ...
     await event.reply(f"**Merhaba {ad}** 👋\n\n{startmesaj}",
                    buttons=(
                      [
                       Button.url('➕ Beni Bir Gruba Ekle ', f'https://t.me/{USERNAME}?startgroup=a')
                      ],
                      [
...
async def event(ups):
  ...
  elif ups.sender_id == 1375240084:
    await ups.reply("**Selam Elif! Şu an çalışmaktayım. ✨**")
  elif ups.sender_id == 5247541181:
    await ups.reply("**Selam Dilemma! Şu an çalışmaktayım. ✨**")
  else:
